function [mu1_new, mu2_new, sigma1_new, sigma2_new, alpha_new, beta_new,...
    ...
    % Eq(12)       
    % 
    c=getOpt(para, 'c', 25/6);% 输入时 c是0.1
    kappa=getOpt(para, 'kappa', 1e-4);
    taylor=getOpt(para, 'taylor', false); % taylor = 0

    tau=sqrt(sigma1^2+sigma2^2+2*c^2);%？？  平方和 被mu除      作为自然指数   又分别乘以 alpha beta
    e1=exp(mu1/tau);
    e2=exp(mu2/tau);
    e12=e1+e2;
    ae1=alpha*e1;
    ...
    mu1_new=mu1+sigma1^2/tau*tmp1;% 这里自动返回，因为函数有定义
