...
saveFile = 'acc_expert_23_4_2_1_OnlineBT.mat';
    ...
    for N = 1 : 1  % times run through the data  N= 5
       %% online Bayesian optimization for tuple (i,j,k) and worker K
        ...
        for r = 1 : budget % n_data
            ...
            k = data(idx(r), 4);
            K = data(idx(r), 1);
           [mu(i), mu(j), sigma(i), sigma(j)]...
                = online_update_OnlineBT(mu(i), mu(j), sigma(i), sigma(j), online_para);
           [mu(i), mu(k), sigma(i), sigma(k)]...
                = online_update_OnlineBT(mu(i), mu(k), sigma(i), sigma(k), online_para);
           [mu(j), mu(k), sigma(j), sigma(j)]...
                = online_update_OnlineBT(mu(j), mu(k), sigma(j), sigma(k), online_para);
